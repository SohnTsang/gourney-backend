// supabase/functions/device-update-preferences/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.notification_preferences || typeof body.notification_preferences !== 'object') {
      return bad(400, 'bad_request', 'notification_preferences object is required')
    }

    const serviceSupabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY)
    
    const { data: device, error: updateError } = await serviceSupabase
      .from('devices')
      .update({
    ...
    if (updateError) {
      if (updateError.code === 'PGRST116') {
        return bad(404, 'not_found', 'Device not found')
      }
