-- 20251003_add_search_places_rpc.sql
-- Add RPC function for complex place search with full-text and filters
-- Handles tsvector queries that PostgREST client can't do directly
  ...
  SELECT 
    ...
    CASE 
      WHEN p_search_text IS NOT NULL THEN
        ts_rank(p.search_tokens, to_tsquery('simple', replace(p_search_text, ' ', '&')))
      ELSE 0.0
    END as search_rank
  FROM public.places p
  WHERE 
    -- Text search (tsvector OR trigram fallback)
    (p_search_text IS NULL OR (
      p.search_tokens @@ to_tsquery('simple', replace(p_search_text, ' ', '&'))
...
COMMENT ON FUNCTION public.search_places IS 
  'Search places with full-text (tsvector), trigram fallback, category/price filters, and cursor pagination';
