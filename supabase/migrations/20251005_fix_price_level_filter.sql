-- 20251005_fix_price_level_filter.sql
-- CRITICAL FIX: price_level parameter was being compared to itself
-- Changed: p.price_level = p_price_level (column = column)
-- To: p.price_level = search_places.p_price_level (column = parameter)
...
BEGIN
  ...
  WHERE 
    ...
    -- FIXED: Explicitly reference the function parameter
...
COMMENT ON FUNCTION public.search_places IS 
  'Search places with full-text, trigram fallback for CJK, category/price filters, cursor pagination. FIXED: price_level parameter reference.';