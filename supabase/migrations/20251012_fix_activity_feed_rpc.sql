-- 20251012_fix_activity_feed_rpc_v2.sql
-- Drop old function and recreate with proper visit data joins
...
-- Recreate with updated return columns
...
BEGIN
  ...
  -- Join visits for ALL activity types that reference a visit
  LEFT JOIN visits v ON a.subject_id = v.id 
    AND a.type IN ('visit', 'visit_comment', 'visit_like')
  LEFT JOIN places p ON v.place_id = p.id
  -- Join comment text for comment activities
  LEFT JOIN visit_comments c ON a.comment_id = c.id
  WHERE
    -- Only show activities for the current user (recipient)
    a.recipient_id = current_user_id
    -- Respect visibility
    AND (
      a.visibility = 'public'
...
COMMENT ON FUNCTION get_activity_feed IS 'Week 5 Step 3: Get personalized activity feed with full visit and comment data';
